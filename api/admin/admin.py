...
@admin_bp.route('/modificar_producto/<int:id>', methods=['GET', 'POST'])
...
@admin_bp.route('/cargar_producto', methods=['GET', 'POST'])
def cargar():
    coneccion = get_connection()
    cursor = coneccion.cursor(dictionary=True)
    data = request.get_json()
...
@admin_bp.route('/borrar_producto/<int:id>', methods=['DELETE'])
def borrar(id):
    coneccion = get_connection()
    cursor = coneccion.cursor(dictionary=True)
